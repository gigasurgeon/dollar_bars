...
batch_size = 1000000
total_batches = int(limit/batch_size)

tick_data = pd.DataFrame()
for idx, batch in enumerate(batched(documents, batch_size)):
    print(f'processing batch: {idx+1}/{total_batches}')
    tick_data = tick_data._append(batch, ignore_index=True)


tick_data['price'] = tick_data['price'].astype(float)
tick_data['qty'] = tick_data['qty'].astype(float)
tick_data['quoteQty'] = tick_data['quoteQty'].astype(float)
tick_data['timestamp'] = pd.to_datetime(tick_data['time'], unit='ms')
...
print(f'limit: {limit}   target_dollar_value: {target_dollar_value}    dollar_bars_generated: {dollar_bars.shape}')
